...
mod4 <- glm(used_bin ~ vo_nest + edgedist, data = mopl_clean, family = binomial)
```
...
-   **What kinds of feedback did you get during week 9 in workshop?:** My peers suggested adding color to represent stress levels, making the icons larger, and finding a clearer way to show social plans. These are all suggestions that I used, because they would make my affective visualization's depiction of the holistic data view easier to understand for the viewer. One classmate also suggested using a phone as the background, which I liked and am going to implement because it fits well with the theme of screen time.
...
screentime <- screentime |> 
  clean_names() |> # making variable names easier to code
  mutate(day_of_week = factor(day_of_week, # allows days to be in order on graph
...
**Figure 1. Screen time appears unrelated to the day of the week** Each point represents the observed screen time (hours) recorded for a specific day, with different colors and shapes used to distinguish the days of the week. Jittered points show the underlying data, while the red circles indicate the mean screen time for each day and the dark red vertical error bars represent the standard error around the mean.
