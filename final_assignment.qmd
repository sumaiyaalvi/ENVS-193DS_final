...
```
**Check 2: Equal variances across groups**
```{r}
leveneTest(screen_time_hours ~ day_of_week, data = screentime)
```
The assumptions of the ANOVA appear to be met. The residual diagnostic plots did not show any clear patterns or strong deviations from normality, suggesting that the residuals are approximately normally distributed. Additionally, the test for equal variances indicated that the variance in screen time is similar across days of the week. Therefore, the assumptions required to run the one-way ANOVA are reasonably satisfied.
...
The effect size for the ANOVA was small ($\eta^2 = 0.04$). This means that only about 4% of the variation in screen time can be explained by the day of the week. Overall, this suggests that the day of the week has very little influence on screen time in this dataset. The results of the one-way ANOVA show that screen time does not significantly differ across days of the week ($F(6,23)=0.152, p=0.987$). Because the p-value is much greater than 0.05, we fail to reject the null hypothesis that mean screen time is the same across all days. This suggests that the day of the week does not appear to have a meaningful effect on screen time in this dataset.
